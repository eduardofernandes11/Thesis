...
\ac{eml} files include all of the raw data that makes up an email, including the headers and the body content.
...
Table~\ref{tbl:c2:comparison_table} summarizes the proposed works discussed in this section related to phishing detection. The literature review revealed several machine learning and deep learning techniques that can be used to detect phishing emails. It also evaluated the performance of various techniques in terms of accuracy. Complementing this analysis, the subsequent table, Table~\ref{tbl:c2:datasets_table}, offers an in-depth overview of the datasets employed in these studies.

\input{figs_tex/c2/comparison_table.tex}
\input{figs_tex/c2/datasets_table.tex}
\input{figs_tex/c2/comparison_table_2.tex}

\newpage

Table \ref{tbl:c2:comparison_table_2} presents a summary of recent works by various authors, the datasets employed, and the corresponding evaluation methods utilized to assess the performance of their proposed classifiers. A common evaluation method observed is the train-test split, with proportions ranging from 80\%/20\% to 70\%/30\%. This method provides a straightforward approach to assess the model on unseen data, ensuring that the classifier can effectively detect phishing attempts beyond the data it was trained on. 
...
\input{figs_tex/c2/emotions_table.tex}
...
% However, it's crucial to evaluate these models using a range of performance metrics beyond mere accuracy. Metrics such as precision, recall, and F1 scores offer a more rounded view of a model's effectiveness, especially in differentiating between phishing and non-phishing emails. The false positive rate is particularly critical in practical settings, as high rates can lead to user dissatisfaction and reduced trust in the system.
